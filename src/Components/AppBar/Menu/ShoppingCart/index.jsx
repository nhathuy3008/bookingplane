...
//         }}
//       >
//         <DialogTitle sx={{ textAlign: "center", fontWeight:"bold" }}>Chỉnh sửa ngày</DialogTitle>
//         <DialogContent
//           sx={{
//             display: "flex",
...
//           <Button onClick={() => setDialogOpen(false)} sx={{ fontWeight: "bold" }}>
//             Hủy
//           </Button>
//           <Button onClick={handleDateChange} variant="contained" sx={{ fontWeight: "bold" }}>
//             Lưu
//           </Button>
//         </DialogActions>
//       </Dialog>
...
const ShoppingCart = () => {
  ...
  useEffect(() => {
    ...
    fetchCartItemsAPI(accountId)
      .then((data) => {
        setProducts(data);
        setSelectedProducts(new Array(data.length).fill(false));
      })
      .catch((error) => {
        toast.error("Không thể tải giỏ hàng.");
        console.error(error);
  ...
  const handleDateChange = () => {
    ...
    updateCartItemAPI(
      updatedProducts[editingIndex]._id,
      updatedProducts[editingIndex].checkInDate,
      updatedProducts[editingIndex].checkOutDate
    )
      .then((updatedCartItem) => {
  ...
  const handleNext = () => {
    ...
    console.log("Cart Data Before Navigate:", cartData); // In ra để kiểm tra
    navigate("/account/checkout", { state: cartData });
};
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          {products.length === 0 ? (
            ...
            products.map((product, index) => (
              <Card key={product._id} variant="outlined" sx={{ p: 2, mb: 2 }}>
                <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1, mb: 1 }}>
                  <Button
                    startIcon={<DeleteIcon />}
                    size="small"
                ...
                <Box sx={{ display: "flex", alignItems: "center" }}>
                  ...
                  <CardContent sx={{ flex: 1 }}>
                    ...
                    <Typography variant="body2" color="text.secondary" mb={1}>
                      <PeopleIcon fontSize="small" sx={{ verticalAlign: "middle" }} />{" "}
                      {product.product.maxGuests} người và <BedIcon fontSize="small" sx={{ verticalAlign: "middle" }} />{" "}
                      {Math.floor(product.product.maxGuests / 2)} phòng ngủ
                    </Typography>
                    <Typography variant="body2" color="text.secondary" mb={1}>
                      <LocationOnIcon fontSize="small" sx={{ verticalAlign: "middle" }} />{" "}
                      {product.product.location.name}, {product.product.location.district.name}, {product.product.location.district.province.name}
                    </Typography>
                    <Typography variant="body2" color="text.secondary" mb={1}>
                      <StarIcon fontSize="small" sx={{ color: "#FFD700", verticalAlign: "middle" }} />{" "}
                      {product.product.rating || "Chưa có đánh giá"} · {product.product.totalReviews || 0} nhận xét
                    </Typography>
