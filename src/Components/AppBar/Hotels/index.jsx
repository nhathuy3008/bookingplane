...
const Hotels = () => {
  ...
  const handleViewDetails = (hotelId) => {
    ...
    navigate(`/account/HotelDetail/${hotelId}`); // Điều hướng đến trang chi tiết
};
...
const handleAddToCart = async (productId) => {
  try {
      ...
      // Kiểm tra nếu checkInDate, checkOutDate hoặc guestCount chưa được chọn
      ...
      let finalCheckInDate = new Date(checkInDate);
      let finalCheckOutDate = new Date(checkOutDate);

      // Kiểm tra xem checkInDate và checkOutDate có giống nhau không
      if (checkInDate.toDateString() === checkOutDate.toDateString()) {
          console.log("Same day booking. Keeping checkOutDate as is.");
          // Giữ nguyên checkOutDate
      }

      // Định dạng ngày theo kiểu YYYY-MM-DD
      const formattedCheckInDate = finalCheckInDate.toLocaleDateString("en-CA");
      const formattedCheckOutDate = finalCheckOutDate.toLocaleDateString("en-CA");

      // Gọi API để kiểm tra xem sản phẩm đã có trong giỏ hàng chưa
      const res = await fetchCartItemsAPI(accountId);
      const cartItems = res || []; // Khởi tạo như một mảng rỗng nếu không có dữ liệu

      // Kiểm tra xem sản phẩm đã có trong giỏ hàng chưa
      const productExists = cartItems.some(
          (item) =>
              item.product === productId && // Chắc chắn rằng bạn so sánh đúng trường
              ...
              <Card key={hotel.id} sx={{ display: "flex", mb: 3 }}>
                <Box sx={{ width: "40%", position: "relative" }}>
                  ...
                  <Box sx={{
                    ...
                    onClick={() => handleViewDetails(hotel._id)} // Sử dụng hotel._id
                >
                    Xem tất cả
                </Button>
                  </Box>
                ...
                <CardContent sx={{ width: "60%", marginTop: "20px" }}>
                  ...
                  <Button variant="contained" sx={{ width: "100%", mt: 3 }}
                    onClick={() => { handleAddToCart(hotel._id); }}>
                    Đặt ngay
                  </Button>
                </CardContent>
              </Card>
