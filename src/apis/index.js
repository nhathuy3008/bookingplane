...
// };
export const addToCartAPI = async (productId, accountId, checkInDate, checkOutDate) => {
  console.log(" Sending request with:", { productId, accountId, checkInDate, checkOutDate });

  if (!productId) {
      console.error(" productId is missing!");
      return; // Nếu productId không có, dừng hàm
  }

  try {
      const response = await axios.post(
          `${URL_API}/api/cart/create`,
          { productId }, // Gửi productId trong body
          {
              params: {
                  accountId,
                  checkInDate,
...
export const addToOrderAPI = async (orderData) => {
  ...
  try {
    ...
    console.error("Error adding to order:", error.response ? error.response.data : error);
